...
DeepSeekConfig parse_deepseek_config(const ModelConfig& config) {
  const auto& j = config.root;
  DeepSeekConfig c;
  c.model_type = ds::util::get_string_or(j, "model_type", config.model_type);
  ...
  if (q_lora != nullptr && !q_lora->is_null()) c.q_lora_rank = static_cast<std::int32_t>(q_lora->as_int());
